 ...
 * 2-satisfability
 * Tested on http://codeforces.com/contest/228/problem/E
 */
#include <bits/stdc++.h>
using namespace std;
...
struct SAT2 {			
	...
	void addOR(int u, int v) { G[NOT(u)].push_back(v); G[NOT(v)].push_back(u); }
	void addIMPLIES(int u, int v) { G[u].push_back(v); G[NOT(v)].push_back(NOT(u)); }
	void addEQUAL(int u, int v) { addIMPLIES(u, v); addIMPLIES(v, u); }
	void addXOR(int u, int v) { addOR(u, v); addOR(NOT(u), NOT(v)); }
